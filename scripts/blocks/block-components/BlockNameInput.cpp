#include "BlockNameInput.h"

BlockNameInput::BlockNameInput(AssetsManager *assets, const std::string& string) {
...
bool BlockNameInput::update(sf::Vector2f mousePos, bool mousePressed, std::vector<int> unicodeEntered) {
    if (mousePressed) {
        ...
        for (int unicode : unicodeEntered) {
            if (unicode < 128) {
                if (unicode != DELETE_KEY && unicode != ENTER_KEY && unicode != ESCAPE_KEY) {
                    if (isalnum(static_cast<char>(unicode))) string << static_cast<char>(unicode);
                    format();
...
void BlockNameInput::draw(sf::RenderTarget &target) {
    target.draw(sprite);
}

sf::Vector2f BlockNameInput::getSize() {
    return {sprite.getGlobalBounds().width, sprite.getGlobalBounds().height};
}

void BlockNameInput::format() {
...
void BlockNameInput::setPos(sf::Vector2f pos) {
    this->pos = pos;
    sprite.setPosition(pos);
}

std::string BlockNameInput::getName() {
    return "name";
}

std::string BlockNameInput::getText() {
    return string.str();
}
